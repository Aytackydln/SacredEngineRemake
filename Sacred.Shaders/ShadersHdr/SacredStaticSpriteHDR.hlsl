...
pixel_output ps_main(vertex_output input)
{
    ...
    if (is_light_halo)
    {
        ...
        halo_output.color = float4(
            SdrTextureToHdr10(input.corner_alpha.rgb, unlit_white_nits) * alpha,
            alpha);
    ...
    if (tex.a < (is_liquid ? (1.0f / 255.0f) : alpha_cutoff))
        discard;

    bool is_unlit = !is_liquid && (input.flags & 0x80000000u) != 0;
    tex.rgb *= is_unlit ? 1.0f : ambient_intensity;
    float nits = is_unlit ? unlit_white_nits : scene_paper_white;

    pixel_output output;
    // Sprite textures contain partially transparent edge texels. Convert their
    // coverage-weighted color in linear light before PQ encoding; converting the
    // unattenuated edge color first turns that coverage into a bright matte in
    // HDR, most visibly along terrain-aligned static sprites.
    output.color = float4(SdrTextureToPremultipliedHdr10(tex.rgb, tex.a, nits), tex.a);
