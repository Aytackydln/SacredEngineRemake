...
static const float animated_texture_emissive_nits = 300.0f;
...
float3 emissive_nits(float4 color, float alpha_scale)
{
    return SrgbToLinear(color.rgb) * (animated_texture_emissive_nits * saturate(alpha_scale));
}
...
float4 ps_animated_main(vs_output input) : SV_Target
{
    ...
    float3 hdr = Linear709NitsToHdr10(emissive_nits(color, color.a));
    return float4(hdr, color.a * model_color.a);
}
